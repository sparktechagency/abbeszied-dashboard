...
import {
  ...
  useUpdateBannerMutation, // Import the correct mutation
...
function Slider() {
  ...
  // API mutations - Use the correct mutation
  const [createBanner, { isLoading: isCreating }] = useCreateBannerMutation();
  const [deleteBanner, { isLoading: isDeleting }] = useDeleteBannerMutation();
  const [updateBanner, { isLoading: isUpdating }] = useUpdateBannerMutation(); // Use correct mutation
  ...
  useEffect(() => {
    ...
    if (Array.isArray(sliderData)) {
      ...
      const formattedData = sliderData.map((item, index) => {
        ...
        const formatted = {
          key: item._id || `temp-${index}`, // Use _id as key
          serial: index + 1,
  ...
  const handleModalSubmit = async (formData) => {
    try {
      ...
      console.log("Submit Data (FormData):", submitData);

      if (isEditing && editingData?._id) {
        // Update existing banner - Use _id directly
        console.log("Updating banner with ID:", editingData._id);
        ...
        await updateBanner({
          id: editingData._id, // Use _id directly
          data: submitData,
        }).unwrap();
  ...
  const onConfirmDelete = async () => {
    try {
      console.log("Deleting banner with ID:", deletingRecord._id);
